...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lbd = do_lambda_form(Pair(formals, body), env)
        env.define(symbol, lbd)
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        return True
    elif len(expressions) == 1 or is_scheme_false(scheme_eval(expressions.first, env)):
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 0:
        return False
    elif len(expressions) == 1 or is_scheme_true(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.first, env)
    else:
