...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if self.has_been_doubled is False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        inspect_place = self.place
        while True:
            if len(inspect_place.bees) > 0 and self.lower_bound <= distance <= self.upper_bound:
                return random_bee(inspect_place.bees)
            else:
                inspect_place = inspect_place.entrance
                distance += 1
                if inspect_place.is_hive:
                    return None
        # END Problem 3 and 4
    ...
    def action(self, gamestate):
        ...
        while behind is not None:
            if behind.ant is not None:
                behind.ant.double()
                if behind.ant.is_container is True and behind.ant.ant_contained is not None:
    ...
    def throw_at(self, target):
        ...
        if target:
            ...
            def slow_action(gamestate):
                nonlocal target_slow_turn
                if target_slow_turn > 0:
                    target_slow_turn -= 1
                    if target_slow_turn > 0 and gamestate.time % 2 == 0:
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        if not self.has_been_scared:
            ...
            def back_action(gamestate):
                if length > 0:
                    if not self.place.entrance.is_hive:
                        destination = self.place.entrance
                        if self.blocked():
                            self.sting(self.place.ant)
                        elif self.health > 0 and gamestate.time % 2 == 0:
                            self.move_to(destination)
                            length -= 1
                else:
                    original_action(gamestate)
            self.action = back_action
