...
def about(subject):
    ...
    def select(paragraph):
        p1 = remove_punctuation(paragraph)
        p2 = lower(p1)
        s = split(p2)
        for characters in s:
            for word in subject:
...
def feline_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    diff_number = 0
    def helper(typed, source, diff_number):
        if diff_number > limit:
            return 1
        elif not typed or not source:
            return max(len(typed), len(source))
        elif typed[0] == source[0]:
            return helper(typed[1:], source[1:], diff_number)
        else:
            diff_number += 1
            return helper(typed[1:], source[1:],  diff_number) + 1
    return helper(typed, source, diff_number)
    # END PROBLEM 6
...
def time_per_word(words, times_per_player):
    ...
    #     time = []
    #     while j < len(times_per_player[i]) - 1:
    #         type_time = times_per_player[i][j + 1] - times_per_player[i][j]
...
def fastest_words(match):
    ...
    for j in word_indices:
        ...
        for k in player_indices:
           if times[k][j] < fastest:
               fastest = times[k][j]
               index = k
        lists[index].append(words[j])
    return lists
    # END PROBLEM 10
